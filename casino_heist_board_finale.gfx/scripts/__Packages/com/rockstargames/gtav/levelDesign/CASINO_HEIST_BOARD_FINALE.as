class com.rockstargames.gtav.levelDesign.CASINO_HEIST_BOARD_FINALE extends com.rockstargames.ui.core.BaseScreenLayout
{
   var BOUNDING_BOX;
   var CONTENT;
   var TIMELINE;
   ...
   function CASINO_HEIST_BOARD_FINALE()
   {
      super();
      _global.gfxExtensions = true;
      this._name = "CASINO_HEIST_BOARD_FINALE";
   }
   function initialise(mc)
   {
      ...
      this.imageManager = new com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.ImageManager(this._name);
      ...
      this.overlay = new com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Overlay(_loc4_);
      ...
      this.cursor = new com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor(_loc3_,_loc2_,this.displayConfig);
      this.cursor.setVisibility(false);
      this.showScreen();
   }
   function ADD_TODO_LIST_ITEM(itemText, isComplete)
   {
      this.screen.addTodoListItem(com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.screens.MainScreen.TODO_LIST_INDEX,itemText,isComplete);
   }
   function CLEAR_TODO_LIST()
   {
      this.screen.clearTodoList(com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.screens.MainScreen.TODO_LIST_INDEX);
   }
   function ADD_OPTIONAL_LIST_ITEM(itemText, isComplete)
   {
      this.screen.addTodoListItem(com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.screens.MainScreen.OPTIONAL_LIST_INDEX,itemText,isComplete);
   }
   function CLEAR_OPTIONAL_LIST()
   {
      this.screen.clearTodoList(com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.screens.MainScreen.OPTIONAL_LIST_INDEX);
   }
   ...
   function SET_CREW_MEMBER(buttonID, name, image)
   {
      this.screen.setCrewMember(buttonID,name,image);
   }
   function SET_CREW_MEMBER_STATE(buttonID, isReady, headsetState)
   {
      this.screen.setCrewMemberState(buttonID,isReady,headsetState);
   }
   function SET_CREW_CUT(buttonID, cut)
   {
      this.screen.setCrewCut(buttonID,cut);
   }
   function SET_SELECTION_ARROWS_VISIBLE(buttonID, visibleState)
   {
      this.screen.setSelectionArrowsVisible(buttonID,visibleState);
   }
   function SET_NOT_SELECTED_VISIBLE(buttonID, isVisible)
   {
      this.screen.setNotSelectedVisible(buttonID,isVisible);
   }
   function SET_HEADINGS(approach, target, setupCost, potentialTake, supportCrewCut, entrance, exit, buyer, outfitIn, outfitOut)
   {
      this.screen.setHeadings(approach,target,setupCost,potentialTake,supportCrewCut,entrance,exit,buyer,outfitIn,outfitOut);
   }
   function SET_CREW_PANEL_VISIBLE(isVisible)
   {
      this.screen.setCrewPanelVisible(isVisible);
   }
   function SET_LAUNCH_BUTTON_LABEL(label)
   {
      this.screen.setLaunchButtonLabel(label);
   }
   function SET_MAP_MARKERS(visibleGroup)
   {
      this.screen.setMapMarkers(visibleGroup);
   }
   ...
   function SET_INPUT_EVENT(inputID)
   {
      ...
      switch(inputID)
      {
         case com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor.UP:
         case com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor.RIGHT:
         case com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor.DOWN:
         case com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor.LEFT:
            _loc3_ = this.cursor.setTarget(inputID);
            if(_loc3_)
            {
               com.rockstargames.gtav.levelDesign.CASINO_HEIST_BOARD_FINALE.playSound("Mouse_Move_Cursor");
            }
            break;
         case com.rockstargames.gtav.levelDesign.CASINO_HEIST_BOARD_FINALE.ACCEPT:
            this.cursor.getTargetIDUnderCursor();
            break;
         case com.rockstargames.gtav.levelDesign.CASINO_HEIST_BOARD_FINALE.CANCEL:
   ...
   function showScreen()
   {
      ...
      {
         this.cursor.setState(com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.Cursor.STATE_ARROW);
         this.screen = new com.rockstargames.gtav.levelDesign.casinoHeistBoardFinale.screens.MainScreen(this,this.screenContainer,this.cursor,this.overlay);
         this.updateButtons();
      }
   }
